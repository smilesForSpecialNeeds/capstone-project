...
class ClientTestimonialList extends Component {
  render() {
    const listOfTestimonials = this.props.testimonial.map(item => <ClientTestimonial key={item.id} testItem={item}/>)
    return (<div>
...
function mapStateToProps(state) {
  return {testimonial: state.testimonialReducer, user: state.userReducer}
}
